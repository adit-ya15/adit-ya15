<div align="center"> 
  <img src="https://banners-opal.vercel.app/api/banner?type=header&text=ADITYA&theme=royal" alt="Header Wave" />
</div> 

<img src="https://komarev.com/ghpvc/?username=adit-ya15&color=00000000" alt="hidden-counter" width="0" height="0" style="display:none !important;" />

<div align="center">
  <a href="https://github.com/adit-ya15">
    <img src="https://futuristic-stats.vercel.app/api?username=adit-ya15&theme=tokyonight&text=ADITYA" alt="Developer Stats" />
  </a>
</div>

<br>
<h2 align="center"> 
  ⚡ &nbsp;&nbsp; 𝐄 𝐍 𝐆 𝐈 𝐍 𝐄 𝐄 𝐑 𝐈 𝐍 𝐆 &nbsp;&nbsp; 𝐒 𝐂 𝐀 𝐋 𝐀 𝐁 𝐋 𝐄 &nbsp;&nbsp; 𝐒 𝐘 𝐒 𝐓 𝐄 𝐌 𝐒 
</h2>
<br>

<div align="center">
  <p>Backend-focused Full Stack Developer building <b>production-ready applications</b><br>
  with real-world deployment, CI/CD pipelines, and scalable architecture</p>
</div>

<br>
<h2 align="center"> 
  🚀 &nbsp;&nbsp; 𝐅 𝐄 𝐀 𝐓 𝐔 𝐑 𝐄 𝐃 &nbsp;&nbsp; 𝐖 𝐎 𝐑 𝐊 
</h2>
<br>

<div align="center">
  <a href="https://github.com/adit-ya15/devsync">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=adit-ya15&repo=devsync&theme=tokyonight" alt="DevSync" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/adit-ya15/BackendProject">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=adit-ya15&repo=BackendProject&theme=tokyonight" alt="Backend Project" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/adit-ya15/music-player">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=adit-ya15&repo=music-player&theme=tokyonight" alt="Music Player" />
  </a>
</div>

<br>
<h2 align="center"> 
  📌 &nbsp;&nbsp; 𝐖 𝐇 𝐀 𝐓 &nbsp;&nbsp; 𝐈 &nbsp;&nbsp; 𝐁 𝐔 𝐈 𝐋 𝐃 
</h2>
<br>

<div align="center">
  <p><b>DevSync</b> → Real-time collaboration platform with CI/CD & AWS deployment<br>
  <b>VideoTube</b> → Scalable backend system for content platform (auth + media + APIs)<br>
  <b>Null Music</b> → Android music player with resilient playback & offline-first design</p>
</div>

<br>
<h2 align="center"> 
  🧠 &nbsp;&nbsp; 𝐄 𝐍 𝐆 𝐈 𝐍 𝐄 𝐄 𝐑 𝐈 𝐍 𝐆 &nbsp;&nbsp; 𝐅 𝐎 𝐂 𝐔 𝐒 
</h2>
<br>

<div align="center">
  <p>⚙️ Backend Systems & API Design<br>
  ☁️ Deployment, CI/CD & DevOps<br>
  📡 Real-time Communication<br>
  📦 Scalable Architectures</p>
</div>

<br>
<h2 align="center"> 
  🛠️ &nbsp;&nbsp; 𝐓 𝐄 𝐂 𝐇 &nbsp;&nbsp; 𝐒 𝐓 𝐀 𝐂 𝐊 
</h2>
<br>

<div align="center">
  <img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,postgres,prisma,react,nextjs,tailwind,ts,redis,aws,nginx,docker,git,github" alt="Tech Stack" />
</div>

<br>
<h2 align="center"> 
  ⚙️ &nbsp;&nbsp; 𝐃 𝐄 𝐕 𝐎 𝐏 𝐒 &nbsp;&nbsp; 𝐒 𝐓 𝐀 𝐂 𝐊 
</h2>
<br>

<div align="center">
  <p>GitHub Actions • AWS EC2 • Nginx • PM2<br>
  CI/CD Pipelines • Deployment Automation • Server Management</p>
</div>

<br>
<h2 align="center"> 
  📊 &nbsp;&nbsp; 𝐆 𝐈 𝐓 𝐇 𝐔 𝐁 &nbsp;&nbsp; 𝐈 𝐍 𝐒 𝐈 𝐆 𝐇 𝐓 𝐒 
</h2>
<br>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=adit-ya15&show_icons=true&theme=tokyonight&hide_border=true" alt="GitHub Stats" />
  &nbsp;&nbsp;&nbsp;
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=adit-ya15&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</div>

<br>
<h2 align="center"> 
  📈 &nbsp;&nbsp; 𝐂 𝐎 𝐍 𝐓 𝐑 𝐈 𝐁 𝐔 𝐓 𝐈 𝐎 𝐍 &nbsp;&nbsp; 𝐀 𝐂 𝐓 𝐈 𝐕 𝐈 𝐓 𝐘 
</h2>
<br>

<div align="center">
  <img src="https://commit-matrix.vercel.app/api?username=adit-ya15" alt="Contribution Matrix" />
</div>

<br>
<h2 align="center"> 
  🏆 &nbsp;&nbsp; 𝐀 𝐂 𝐇 𝐈 𝐄 𝐕 𝐄 𝐌 𝐄 𝐍 𝐓 𝐒 
</h2>
<br>

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=adit-ya15&theme=algolia&no-frame=true&row=1&column=6" alt="Trophies" />
</div>

<br>
<h2 align="center"> 
  🐍 &nbsp;&nbsp; 𝐂 𝐎 𝐍 𝐓 𝐑 𝐈 𝐁 𝐔 𝐓 𝐈 𝐎 𝐍 &nbsp;&nbsp; 𝐒 𝐍 𝐀 𝐊 𝐄 
</h2>
<br>

<p align="center">
  <img src="https://raw.githubusercontent.com/adit-ya15/adit-ya15/royal-assets/royal-snake.svg" alt="GitHub Contribution Snake" />
</p>

<br>
<h2 align="center"> 
  🤝 &nbsp;&nbsp; 𝐋 𝐄 𝐓 ' 𝐒 &nbsp;&nbsp; 𝐂 𝐎 𝐍 𝐍 𝐄 𝐂 𝐓 
</h2>
<br>

<div align="center">
  <a href="https://www.linkedin.com/">
    <img src="https://skillicons.dev/icons?i=linkedin" height="40" alt="LinkedIn" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="mailto:your-email@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" height="40" alt="Gmail" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://null-music.netlify.app/">
    <img src="https://img.shields.io/badge/🚀%20Try%20Null%20Music-000?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Try Null" />
  </a>
</div>

<br>

<div align="center"> 
  <img src="https://banners-opal.vercel.app/api/banner?type=footer&theme=royal" alt="Footer Wave" />
</div>
